...
- **后端**：`src/web/`  
  - `service.py`：`ConversationService` 封装单轮对话，内部调用 **`brain.conversation.run_one_turn_stream`**（Mem0 检索 → 截屏或 override → 主脑流式），历史与 CLI **共用唯一数据源**（`chat_history_store`，即 config 中的 `chat_history_file`，默认 `data/chat_history.json`）。支持 `vision_image_override`，供心跳触发的主动回合使用。  
  ...
  - **日志**：与 CLI 共用 `config` 中的 `log_dir` / `log_file`（默认 `logs/soulseed.log`），启动、心跳开启状态、心跳触发与异常均写入该文件。  
...
1. 确保已配置 `.env` 中的 `LITELLM_API_KEY`、`OPENAI_BASE_URL`、`CHAT_MODEL`（主脑、Mem0、语音转写统一走 `llm-gateway`）。
2. **一键起停（推荐）**  
   在项目根目录执行：
   ```bash
...
- **GET /api/config**：返回前端所需配置（如 `tts_reply_enabled`），不暴露敏感项。
...
- **POST /api/speech-to-text**（语音输入）  
  ...
  - 响应：`{"text": "识别文本"}`。通过 `llm-gateway` 调用 LiteLLM（可观测性由 Langfuse callback 提供）；未配置或失败时 `text` 可为空。
- **POST /api/tts**（嘴巴 TTS）  
  - 请求体：`{"text": "要读的句子"}`。  
  - 响应：`audio/mpeg` 音频流（Edge-TTS 合成）。当 config **`tts_reply_enabled`** 为 true 时，前端在助手回复流式结束后自动解析「说的话」并依次请求本接口播放；为 false 时不播报。
...
| brain.conversation | 使用，`run_one_turn_stream`（单轮流水线：Mem0 + 视觉 + 主脑流式）。 |
| memory     | 使用，`add_background`（流水线内 Mem0 检索由 conversation 调用）。 |
| vision     | 使用，`get_screen_for_turn`（通过 ConversationService 传入 `get_vision_image`）。 |
...
前端通过 `GET /api/config` 获取，用于控制行为（不暴露敏感项）：

| 配置项 | 说明 |
|--------|------|
| `tts_reply_enabled` | 是否在助手回复流式结束后自动播报「说的话」；false 时仅文字展示 |

音色等由后端使用：`tts_voice`（Edge-TTS 音色名，如 `zh-CN-XiaoxiaoNeural`）。
...
- **日志**：所有日志（含 `[src.web.server]`、`[src.senses.vision]` 等）写入 config 指定的 `log_dir`/`log_file`（默认 `logs/soulseed.log`），与 CLI 同文件。启动时会打「Web 模式启动」「眼睛心跳: 已开启/未开启」；心跳触发时会打「心跳触发主动说话」「心跳主动回合已写入历史」。
